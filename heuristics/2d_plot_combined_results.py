...
file_dict={
            ...
            "EIS_functional" :{"params":["k_0",  "Cdl", "alpha", "cpe_alpha_faradaic", "Ru"],"burn":5000, "true_vals":True,"colour":colours[3], "label":"EIS"},  
            "Harm_min_Ru_fitting":{"params":["k_0", "alpha"],"burn":5000,"true_vals":True, "colour":colours[2], "label":"Harmonic minimum"}, 
            "Trumpet_no_Ru_fitting":{"params":["E_0", "k_0", "alpha"],"burn":2500,"true_vals":True, "colour":colours[1], "label":"Trumpet plot"},     
...
axis=mplot.plot_2d(file_dict[desired_file]["params"], chains, burn=file_dict[desired_file]["burn"], order=file_dict[desired_file]["order"], rotation=30,  pooling=True, density=False)
    ...
    for i in range(0, len(file_dict[key]["params"])):
        for j in range(0, len(file_dict[key]["params"])):
            ...
            if i==j:
                ...
                axis[idx_i, idx_i].hist(pooled_chains[i], density=False, alpha=0.8, color=file_dict[key]["colour"])
            else:
                idx_j=master_param_list.index(file_dict[key]["params"][j])
                print(idx_i, idx_j, )
                if idx_i>idx_j:
                    axis[idx_i, idx_j].scatter(pooled_chains[j], pooled_chains[i], s=0.5, alpha=0.5, color=file_dict[key]["colour"])

for i in range(0, len(master_param_list)):
    for j in range(0, len(master_param_list)):
        if i==j:
            axis[i,i].axvline(true_values[master_param_list[i]], color="black", linestyle="--", lw=1)     
        elif i>j:
