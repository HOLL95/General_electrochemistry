...
class Laviron_EIS(single_electron):
    ...
    def simulate(self, parameters, frequencies):
        ...
        k0=self.nd_param.nd_param_dict["k_0"]
        e0=self.nd_param.nd_param_dict["E_0"]
        alpha=self.nd_param.nd_param_dict["alpha"]
        gamma=self.nd_param.nd_param_dict["gamma"]
        area=self.nd_param.nd_param_dict["area"]
        dc_pot=self.simulation_options["DC_pot"]/self.nd_param.c_E0
        ratio=np.exp(e0-dc_pot)
        red=gamma/(ratio+1)
        ox=gamma-red
        i_app=(np.exp(self.FRT*(dc_pot-e0))/(1+np.exp(self.FRT*(dc_pot-e0)))**2)/self.nd_param.c_I0
       
        Ra_coeff=k0#(self.R*self.T)/((self.F**2)*area*k0)
        nu_1_alpha=np.exp((1-alpha)*(dc_pot-e0-(i_app*self.nd_param.nd_param_dict["Ru"])))
       
        nu_alpha=np.exp((-alpha)*(dc_pot-e0-(i_app*self.nd_param.nd_param_dict["Ru"])))
        #print((-alpha)*self.FRT*(dc_pot-e0-(i_app*self.nd_param.nd_param_dict["Ru"])))
        #print(nu_alpha, nu_1_alpha)
        Ra=Ra_coeff*((alpha*ox*nu_alpha)+((1-alpha)*red*nu_1_alpha))**-1
        sigma=k0*Ra*(nu_alpha+nu_1_alpha)
        Cf=1/sigma

        EIS_params={}
        EIS_params["R0"]=self.nd_param.nd_param_dict["Ru"]
        EIS_params["R1"]=Ra#self.nd_param.nd_param_dict["R2"]
        
        if self.simulation_options["EIS_Cdl"]=="C1":
            EIS_params["C1"]=self.nd_param.nd_param_dict["Cdl"]
        else:
        ...
        if self.simulation_options["EIS_Cf"]=="C2":
            ...
            EIS_params["Q2"]=Cf**(self.dim_dict["cpe_alpha_faradaic"]-1)*Ra**(-1-self.dim_dict["cpe_alpha_faradaic"])
        ...
        #Z_vals=np.multiply(Z_vals, self.nd_param.c_E0/self.nd_param.c_I0 )
