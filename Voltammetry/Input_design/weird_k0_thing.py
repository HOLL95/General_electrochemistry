...
param_list={
    ...
    'Cdl': 4e-4, #(capacitance parameters)
    'CdlE1': 0.00653657774506,
...
for file in files:
    ...
    for i in range(4, len(results_dict["param_values"]) ):
        ...
        for j in range(1, 2):
            ...
            sim_farad_params=copy.deepcopy(ref_farad_params)
            for z in range(0, 1):#len(param_ranges[farad_params[j]])):
                if farad_params[j]=="E_0":
                    value=sim.un_normalise(param_ranges[farad_params[j]][z], param_bounds["E_0"])
                else:
                    value=param_ranges[farad_params[j]][z]
                ...
                sim_farad_params[j]=value
                print(sim_farad_params, farad_params[j])
                for q in [0, sim.dim_dict["num_frequencies"]]:
                    phases=[0.25229591343617364, 1.3177824914484195, 0.0185365510105075, 0.8710434585224034, -0.4741814547428456]
                    for p in range(0, q):
                        idx=results_dict["params"].index("phase_{0}".format(p+1))
                        results_dict["param_values"][i][idx]=phases[p]
                    current=sim.i_nondim(sim.test_vals(np.append(sim_farad_params,results_dict["param_values"][i]), "timeseries"))*1000 

                    if q==0:
                        desired_times=np.where((times>0.4135) & (times<0.4155))
                    else:
                       desired_times=np.where((times>0) & (times<0.002))
                    #ax.plot(times[desired_times], current[desired_times], label=label)
                    #twinx=ax.twinx()
                    voltages=sim.define_voltages(individual_sines=True)
                    for volt in voltages:
                        if q==0:
                            ax.plot(sim.e_nondim(volt)[desired_times], linestyle="--")
                        else:
                            ax.plot(sim.e_nondim(volt)[desired_times], linestyle="-")
            plt.show()
        ...
        plt.show()#
        fig.savefig("Odd_k0_effect", dpi=500)