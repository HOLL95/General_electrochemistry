        ...
        for j in range(0, len(farad_params)):
            ...
            for z in range(0, len(param_ranges[farad_params[j]])):
                ...
                if comparator=="pc":
                    ...
                    pc_diff[z]=np.log10(100*(sim.RMSE(shifted_current, current)/np.mean(np.abs(current))))
                elif comparator=="FIM":
                
                    S1=Sensitivity(param_list, simulation_options, other_values, param_bounds)
                    S1.simulation_options["method"]="sum_of_sinusoids"
                    S1.def_optim_list(all_param_names)
                    current=S1.test_vals(np.append(sim_farad_params,f_vals), "timeseries")
                    #plt.plot(current)
                    current, theta, potential=S1.simulate_S1()
                    #plt.plot(current)
                    #print(len(current))
                    #plt.show()
                    pc_diff[z]=S1.D_optimality(noise=1)
                    print(pc_diff[z])
                    plt.plot(current)
                    plt.show()
            ...
            if i<2:
                ax.plot(param_ranges[farad_params[j]], pc_diff, linestyle="--")
                
            else:
                ax.plot(param_ranges[farad_params[j]], pc_diff, label=i-1)
            ax.set_xlabel(titles[j])
            if comparator=="pc":
                ax.set_ylabel("log10(%) w/ $\Delta${0} {1}".format(units[j], shift_vals[j]))
            if comparator=="FIM":
                ax.set_ylabel("det(FIM)")
ax.legend()
