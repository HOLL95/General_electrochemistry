...
for j in range(0, len(models)):
    for k in range(0, len(repeats)):
        ...
        labels=[]
        print("{0} Repeat {1}".format(models[j], k+1))
        print("Cocentration, R1")
        for i in range(0, len(concentrations)):
            ...
            new_chain_conced=mplot.concatenate_all_chains(new_chain)

            print(" {0}, {1}".format(concentrations[i],np.mean(new_chain_conced, axis=1)[1]))
            plot_chain=np.log10(new_chain)
            if i==0:
                ax,twinx=mplot.plot_2d(common_params,plot_chain, label=repeats[k], burn=mplot.options["burn"], axis=ax,  alpha=0.1, title_debug=True, pooling=True, colour=colours[i])
            else:
                mplot.plot_2d(common_params,plot_chain, label=repeats[k], burn=mplot.options["burn"], axis=ax,  alpha=0.1, title_debug=True, pooling=True, twinx=twinx, rotation=45, colour=colours[i])
        mplot.axes_legend(labels,ax[0,-1], colours=colours)
        fig.set_size_inches(10, 10)
        plt.subplots_adjust(bottom=0.125, left=0.136, right=0.91, top=0.975)
        fig.savefig("2D_MCMC_by_repeat_{0}_logscale_{1}.png".format(repeats[k], models[j]), dpi=500)
