...
directions=[-1,1, -1, 1]
mplot=MCMC_plotting()
with open("LPMO_ph5_results.txt", "w") as filehandle:
    for i in range(0, len(files)):
        filehandle.write(scan_names[i]+"\n")
        fig,ax=plt.subplots()
        file=files[i]
        ...
        for j in range(0, len(extra_terms)):
            ...
            """sim_current=SW.simulate([0.022267784664755125,1e-5, 0.18255366647910262, 5.617297474936013e-10, 0.6911063950573322, 0.8114990960863366], [])
            plt.plot(E_p,sim_current, )
            sim_current=SW.simulate([0.022267784664755125,0.03, 0.18255366647910262, 5.617297474936013e-10, 0.6911063950573322, 0.8114990960863366], [])
            ...
            micro_error=copy.deepcopy(found_parameters[-1])*1e6
            param_line=[str(x) for x in list(found_parameters)]
            param_line[-1]=str(micro_error)
            ...
            filehandle.write((" ").join(param_names+["\n"]))
            filehandle.write((" ").join(param_line+["\n"]))
            results_dict[extra_terms[j]]=sim_current*1e9
            #plt.plot(E_p, experimental_current)
            #plt.plot(E_p, sim_current)
            #plt.show()
