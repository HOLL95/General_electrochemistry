...
param_file=open("Ella_set_2_params.txt", "w")
for i in range(0, len(files)):
    for j in range(0, len(Ru_vals)):
        ...
        #sim=trumpets.simulate(init_vals1, in_volts, optimise_flag=True)
        #
        ...
        for m in range(0,2):
            ...
            cmaes_fitting.set_max_unchanged_iterations(iterations=100, threshold=1e-2)
            ...
            if found_value>best_score:
                best_params=real_params
        param_file.write("%s, %s Ohms, %s \n"%(files[i], param_list["Ru"], best_params))
