...
import copy
import time
scale_vals=[0.5, 0.75, 1.0, 1.25]
std_vals=[0.01, 0.03, 0.04, 0.05]
...
disp_bins=[1, 2, 20, 30, 40]
for i in range(0,len(vals)):
    for j in range(0, len(classes)):
        if j==0:
            classes[j].simulation_options["dispersion_bins"]=[125]
            param_list["Cdl"]=1e-6
        else:
            classes[j].simulation_options["dispersion_bins"]=[125]
            lav_cdl_val= param_list["Cdl"]*param_list["area"]
            param_list["Cdl"]=lav_cdl_val
        axes=ax[i,j]
        twinx=axes.twinx()
        if i==0:
        ...
        for z in range(0, len(vals[i])):
            #classes[j].simulation_options["dispersion_bins"]=[disp_bins[z]]
        ...
        #classes[j].def_optim_list(["k_0", "Cdl", "E_0"])
        #bode_vals=classes[j].simulate([param_list[x] for x in classes[j].optim_list], frequencies*2*math.pi)
        #EIS().bode(bode_vals, frequencies, ax=axes, twinx=twinx, compact_labels=True, data_type="phase_mag")
