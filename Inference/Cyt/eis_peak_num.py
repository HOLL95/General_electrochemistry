...
twinx=ax.twinx()
for n_p in num_peaks:
    for sf in [8]:#, 9, 10, 11]:
        param_list["num_peaks"]=n_p
        param_list["sampling_freq"]=(1/2**sf)
        ...
        sim_vals=[DC_val,1e-10, 10,1e-5, 0.55, 250, 0,0]
    
        sim=td.simulate(sim_vals, freqs)
        EIS().bode(sim, freqs, ax=ax, twinx=twinx, label=sf, data_type="phase_mag")
plt.show()
