...
modified_best_cdlcpe={'gamma': 2.0452710068414738e-09, 'k_0': 6.445392860387815, 'Cdl': 0.00010679950285178704, 'alpha': 0.6455286284473415, 'Ru': 92.34006414011617, 'cpe_alpha_cdl': 0.5744311935370501, 'cpe_alpha_faradaic': 1}
...
modified_best_neither={'gamma': 3.069616091890602e-09, 'k_0': 7.747287787538096, 'Cdl': 5.005944779664158e-06, 'alpha': 0.6095578924032948, 'Ru': 109.74154304677644, 'cpe_alpha_cdl':1, 'cpe_alpha_faradaic': 1}
...
fig, ax=plt.subplots()
twinx=ax.twinx()
cpe_val=0.7278632452536045
for cpe in [0.7, 0.8, 0.9, 1]:
    window_val="15"
    
    DC_val=-0.2850
    modified_best_neither["cpe_alpha_faradaic"]=cpe
    values=[modified_best_neither[x] for x in laviron.optim_list]
    sim_data=laviron.simulate(values, frequencies)

   
    EIS().bode(sim_data, frequencies,ax=ax, twinx=twinx, data_type="phase_mag", scatter=1, label=cpe)
