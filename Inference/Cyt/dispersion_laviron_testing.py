...
#laviron.def_optim_list(["gamma","k_0",  "Cdl", "alpha", "Ru", "E0_mean", "E0_std"])
...
for shape in [0.25, 0.5, 0.75, 1.0, 1.25]:
    ...
    EIS().bode(lav_ec_vals, frequencies, ax=axes[1], twinx=twinx2, label=shape)

for i in range(0, len(laviron.disp_test),3):
    EIS().bode(laviron.disp_test[i]*np.prod(laviron.weights[i]), frequencies, ax=axes[2], twinx=twinx_3)
