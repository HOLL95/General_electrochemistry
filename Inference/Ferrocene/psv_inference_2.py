...
#time_series_params2=[0.2880815339698446, 0.05860420948850217, 1595.2520017503782, 376.39459016561597, 4.9923089914481455e-05, 0.09978506938843376, 7.381795716927277e-05, 7.233639891663934e-05, 7.41597533294682e-11, 9.015059356029282, 4.6278044953845585, 4.9531469215010615, 0.45652545794383387]
#time_series_params2=[0.2647913851657289, 0.06589851488141335, 51.629709412826564, 79.7678120958783, 5e-4, 0.03453700285002159, 0.00020290154443842296, 9.988448432662733e-05, 6.746515858716504e-11, 9.014965195066791, 3.7701397431132753, 4.924699088331275, 0.5993230263084472]
time_series_params2=[0.25919950037331174, 0.06805648311088913, 198.75368572912876, 88.07303113568388, 0.00034823891990810207, 0.02420675936492858, -0.0004999982190308525, 2.561143319297091e-05, 6.061113052117431e-11, 9.01505671602343, 5.5965024217788315, 4.957512738068864, 0.5999999603113891]
timeseries_params2=[0.2535789476618048, 0.058273117992579615, 75.8915855182773, 69.9957171218355, 0.00022729766251380196, -0.012269766049442632, -0.0004973333110912874, 3.582339061809169e-05, 5.878346854035305e-11, 9.015086163376436, 3.777979297600853, 4.922691701307663, 0.5999998714347011]

test1=ferro.test_vals(time_series_params, "timeseries")
test2=ferro.test_vals(time_series_params2, "timeseries")
h_class.plot_harmonics(time_results, current_time_series=current_results,exp_time_series=test1,  exp2_time_series=test2,  xaxis=ferro.define_voltages(no_transient=True), hanning=False)
plt.show()
plt.plot(ferro.define_voltages(no_transient=True), test2)
plt.plot(voltage_results, current_results)
plt.show()
plt.plot(fourier_arg)
plt.plot(ferro.top_hat_filter(test1), alpha=0.7)
plt.plot(ferro.top_hat_filter(test2), alpha=0.7)
plt.show()
dataset=[current_results, test1, test2]
dt=time_results[1]-time_results[0]
for i in range(0, len(dataset)):
    plt.plot(voltage_results, h_class.inv_objective_fun(dataset[i], dt=dt))
