...
h_class=harmonics(list(range(1, 9)),9.036368906531866, 0.05)
dec_amounts=[16]
...
for i in range(0, len(dec_amounts)):
    ...
    h_class.plot_harmonics(curr_dict["time"], exp_time_series=curr_dict["current"], plot_func=abs, hanning=True)
    plt.show()
...
h_class=harmonics(list(range(1, 9)),8.794196510802587, 0.05)


ferro.def_optim_list(["E0_mean", "E0_std","k_0","Ru","Cdl","CdlE1", "CdlE2", "CdlE3","gamma","omega","cap_phase","phase", "alpha"])
time_series_params=[0.2591910307724134, 0.0674086382052161, 177.04633092062943, 88.31972285297374, 0.000342081409583126, 0.02292512550909509*0, -0.0004999993064740369*0, 2.5653514370132974e-05*0, 6.037508022415195e-11, 8.794196510802587, 0, 0, 0.5999998004431891]
...
#plt.plot(sim)
plot_args=dict(sim_time_series=sim, sim2_time_series=sim2,data_time_series=ferro.i_nondim(current_results)    ,hanning=True, plot_func=abs)#
