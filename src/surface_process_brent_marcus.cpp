...
double Marcus_kinetics(double Normalised_E,double Upper_lambda, int flag){
    ...
    double result, error;
    gsl_integration_qags(&F, -50, 50, 0, 1e-7, 1000, w, &result, &error);
...
struct e_surface_fun {
    ...
    e_surface_fun (
        ...
        integral_0_reduction(integral_0_reduction),
        () { }
...
py::object brent_current_solver(py::dict params, std::vector<double> t, std::string method, double debug=-1, double bounds_val=10, bool Marcus_flag=False) {
    const double v=1;
    ...
    double Upper_lambda=get(params,std::string("Upper_lambda"),0.0);
    ...
    for (int n_out = 0; n_out < t.size(); n_out++) {
        while (t1 < t[n_out]) {
            ...
            e_surface_fun bc(E,dE,cap_E,Cdl,CdlE,CdlE2,CdlE3,E0,Ru,k0,alpha,Itot0,u1n0,dt,gamma, Upper_lambda);
            ...
            std::pair <double, double> sol=boost::math::tools::brent_find_minima(bc,Itot0-Itot_bound,Itot0+Itot_bound, digits_accuracy, max_it, integral_0_oxidation, integral_0_reduction);
