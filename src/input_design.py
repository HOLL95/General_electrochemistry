...
class Input_optimiser(single_electron):
    ...
    def sobol_simulate(self, parameters):
        ...
        #print(list(normed_params), list(parameters))
        ...
        ts_len=len(pot)-1
        self.ts_store=np.zeros((len_sample_values,ts_len))
        num_cpu=mp.cpu_count()
        for i,row in enumerate(param_mat):
            self.sim_callback(self.ts_wrapper(i, row))
            #time_series_matrix[i, :]=self.test_vals(param_mat[i,:], "timeseries")[1:] #row is parameter variation, column is timepoints
        print(time.time()-start, "sim_time")
        time_series_matrix=self.ts_store
        sobol_1=np.zeros(( len(self.simulation_options["sobol_params"]), ts_len))
        variance=np.zeros(ts_len)
        
        for i in range(0, ts_len):
            Si=sobol.analyze(problem, time_series_matrix[:,i], parallel=False, n_processors=num_cpu)#Calculating sobol indices for every parameter (row), over iterative timepoints (col) 
            sobol_1[:,i]=Si["S1"]
    ...
    def sim_callback(self, result):
        self.ts_store[result[0], :]=result[1][1:]