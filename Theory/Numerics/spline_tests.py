...
for SR in np.flip([25, 50, 100, 133, 200, 400]):
    param_list={
        "E_0":0.2,
    ...
    if SR==400:
        ...
        orig_current=sim.test_vals([], "timeseries")
        orig_time=sim.time_vec
        continue
    else:
        interval=400//SR
        sampled_current=orig_current[::interval]
        sampled_time=orig_time[::interval]
    ...
    for i in range(0, len(noise_vals)):
        axis=ax[i//2, i%2]
        noisy_sampled_current=sim.add_noise(sampled_current, noise_vals[i]*max(orig_current))
        
        if residuals==True:
            for method in methods:
                start=time.time()
                interpolated_current=general_interp(orig_time, sampled_time, noisy_sampled_current, method)
                residual=np.subtract(orig_current, interpolated_current)
                print(method, time.time()-start)
                axis.plot(orig_time, residual, label=method, alpha=0.7)
            #axis.plot(orig_time, orig_current)
        else:    
