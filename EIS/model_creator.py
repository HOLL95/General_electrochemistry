...
for i in range(0, num_cols):
    ...
    if len(cpe_vals)!=0:
        for j in range(0, len(cpe_vals)):
            ...
            str_exp=re.sub(r'I_V1\*h{0}|h{0}\*I_V1'.format(cpe_num), "self.cpe(self.t_array[:self.t_counter], self.cpe_arrays[\"cpe_{1}\"][:self.t_counter], self.params[\"Q{1}\"], self.params[\"alpha{1}\"])".format(index, cpe_num), str_exp)
    if "V1" in str_exp:
...
dt=0.001
t_end="dt*1000"
source_func="0.03*np.sin(math.pi*2*8*t)"
params="{\"R1\":1, \"C1\":1e-3, \"Q1\":1, \"alpha1\":0.5}"
for line in read_eq:
