...
for voltage in voltages:
    ...
    #EIS().nyquist(spectra)
    #plt.show()
interesting_combos=[(1,0), (1,4), (2,0), (2,1), (3,4), (4,0)]
for i in range(0, len(interesting_combos)):
        file_name="Best_candidates/Cjx183/{1}/{2}/Best_candidates_dict_{0}_12_gen_truncated_{3}.npy".format(interesting_combos[i][0], method, "5mV", truncate_amount)
        results_dict=np.load(file_name, allow_pickle=True).item()

        #print(results_dict[0]["scores"])
        j=interesting_combos[i][1]
        translator=EIS()
        simulator=EIS_genetics()
        translated_circuit, params=translator.translate_tree(results_dict["models"][j], get_param_list=True)
        print(dict(zip(params, results_dict["parameters"][j])))
