...
for file in files:
    if exp in file:
        data=read_csv(data_loc+file, sep=",", encoding="utf-16", engine="python", skiprows=6, skipfooter=1)
print(data.columns)
numpy_data=data.to_numpy(copy=True, dtype='float')
print(numpy_data)
frequencies=np.flip(numpy_data[:-1, 0])
sim=np.column_stack((np.flip(numpy_data[:-1, 4]),-1*np.flip(numpy_data[:-1, 5])))
print(frequencies[0]*1j)
print(sim)
