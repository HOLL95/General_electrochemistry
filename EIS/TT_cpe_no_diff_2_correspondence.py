...
R_ohm = 10.0       ;    K_dl =  0.0005    ;    gamma = 0.6     ;   kp    = 1000        ;    kmp   =  0.001
...
for kk in range(len(freqs)):
    ...
    for m in range(len(E)-1):
        ...
        if m > 0:
            ...
            if m == 1:
                def Rekur(x):
                    kf=kzero*np.exp((1-alpha)*n*F*(E_t-E_0-R_ohm*x)/(R*T))
                    kb=kzero*np.exp(-alpha*n*F*(E_t-E_0-R_ohm*x)/(R*T))
    ...
    #z=np.fft.fft(potential)/np.fft.fft(current)
    ...
    #conv_current=test_class.i_nondim(test_class.simulate_current(CPE=False))
    #potential=test_class.e_nondim(test_class.define_voltages())
    #times=test_class.t_nondim(eis_test.time_vec)
    #fft_freq_2=np.fft.fftfreq(len(conv_current), times[1]-times[0])
    #plt.plot(t, I_ges)
    #plt.plot(times, conv_current, linestyle="--")
    #plt.show()
    #plt.subplot(1,2,1)
...
"""
 param_list={
    ...
    param_list["original_omega"]=freq
    param_list["omega"]=freq
    ...
    E=eis_test.e_nondim(eis_test.define_voltages()[eis_test.time_idx])
    times=eis_test.t_nondim(eis_test.time_vec[eis_test.time_idx])
    """