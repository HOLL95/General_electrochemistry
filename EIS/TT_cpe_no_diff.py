...
A     =  0.1       ;    c    =  0.001        ;    nu    =  0.1    ;   alpha =  0.5        ;    kzero =  1000#0.001
...
U_amp     =300e-3          # Excitation amplitude for EIS signal
...
param_list={
    "E_0":E_0,
    'E_start':  E_i, #(starting dc voltage - V)
    'E_reverse':E_i+2*U_amp,
...
for kk in range(len(freqs)):
    ...
    omega     = 2*np.pi*freq
    orig_param_list=copy.deepcopy(param_list)
    orig_param_list["omega"]=freq
    orig_param_list["original_omega"]=freq
    test_class=conv_model(None, dim_parameter_dictionary=orig_param_list, simulation_options=sim_options)
    ...
    for m in range(len(E)-1):
        ...
        if m > 0:
            ...
            if m == 1:
                def Rekur(x):
                    return (  (  ( kzero*(1-theta)*np.exp((1-alpha)*n*F*(E_t-E_0-R_ohm*x)/(R*T)) -kzero*(theta)*np.exp(-alpha*n*F*(E_t-E_0-R_ohm*x)/(R*T))  )
    ...
    #=================================================================================
    #=================================================================================
    plt.plot(E[PPP:-PPP], I_ges[PPP:-PPP])
    plt.show()
    #plt.plot(I_f)
    #plt.show()
    ...
    #plt.plot(t, I_ges)
    #ax=plt.gca()
    #ax.twinx().plot(time,current, color="red")
    #plt.show()
...
#plt.show()
    ...
    if kk == 25:
        ...
        plot1.scatter(1000*t[0:-1], 1000*I_f[0:-1], color = 'blue', linewidth = 1, linestyle = '-' , label = '$I_{\mathrm{F}}$')
...
plot2.scatter(FT_Z.real, -FT_Z.imag, color ='black', linewidth=1, linestyle ='-')
