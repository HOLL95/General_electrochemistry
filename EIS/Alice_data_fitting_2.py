...
data_loc="Experimental_data/Alice_15_8_22/"
...
for z in range(0, len(file_nums)):
    ...
    truncate=-35
    real=np.flip(numpy_data[:truncate, 4])
    imag=-np.flip(numpy_data[:truncate,5])
    ...
    for methods in ["AIC"]:
        ...
        for i in range(0, 5):
            ...
            np.save("Best_candidates/{2}/{0}/{4}/testy_alice".format(methods, i+1, exp, abs(truncate), voltage),{})
            ...
            np.save("Best_candidates/{2}/{0}/{4}/Best_candidates_dict_{1}_12_gen_truncated_{3}_alice.npy".format(methods, i+1, exp, abs(truncate), voltage), gene_test.best_candidates)
