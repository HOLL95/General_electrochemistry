...
def PlanarSemiinfImpCalculator(freqs, Ru, EZero, Eeq, Dred, Dox, Cap, kp, kmp, kf, kmf, gamma):
    ...
    return Z
def NodiffImpCalculator(freqs, Ru,gamma, Cap, alpha, kf_el, kb_el, theta, ctot):
    print(kf_el, kb_el)
    Freq    = 2*np.pi*freqs
    R_u     = Ru
    coeff=-(A*F**2)/(R*T)
    ZFar    =coeff*(-alpha*kf_el-((1-alpha)*kb_el))
    print(ZFar)
    #----------------------------------------------------------------------------------------------
    Z  = R_u + 1/((1.0/ZFar) + (((1j*Freq)**gamma)*Cap))
    return Z
...
A     =  0.07       ;    c    =  0.001        ;    nu    =  0.1    ;   alpha =  0.5        ;    kzero =  0.001
...
theta_0=1
kf=kzero*np.exp((1-alpha)*n*F*(E_i-E_0)/(R*T))
kb=kzero*np.exp(-alpha*n*F*(E_i-E_0)/(R*T)) 
theta_top=kf+(theta_0*kf+theta_0*kb-kf)
theta=theta_top/(kf+kb)
#Z_classical=NodiffImpCalculator(freqs, Ru=R_ohm,gamma=gamma, Cap=K_dl, alpha=alpha, 
#                                kf_el=kf, 
#                                kb_el=kb,
#                                theta=theta,
#                                ctot=1e-9)
#plt.plot(Z_classical.real, -Z_classical.imag)
#plt.show()
