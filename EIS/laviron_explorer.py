...
for i in range(0, len(circuit_list)):
    ax=axes[i]
    circuit=circuit_list[i]
    for dc_pot in [-0.01, 0, 0.02]:   
        k0=200
        #dc_pot=0
        gamma=4e-10 
        alpha=0.5
        ...
        test.nyquist(spectra, orthonormal=False, scatter=1,ax=ax, label="$E-E^0$={0} mV".format(1000*(dc_pot-e0)))
    ax.legend()
plt.show()
