...
def current(cdl,amp,frequency, time, phase):
    return (cdl)*amp*frequency*np.cos(2*np.pi*frequency*time+phase)
...
def impedance_response(min_f, max_f, points_per_decade, num_osc, parameters,sim_class, sf=200, amplitude=5e-3, noise=None):
    ...
    for i in range(0, len(frequency_powers)):
        ...
        #plt.plot(times, V)
        #plt.show()
        I=current(conv_class.dim_dict["Cdl"],amplitude,freqs[i], times1,conv_class.dim_dict["cap_phase"])
        ...
        #I=conv_class.i_nondim(conv_class.nd_param.nd_param_dict["Cdl"]*np.array([conv_class.voltage_query(x)[1] for x in conv_class.time_vec[conv_class.time_idx]]))
        #plt.plot(times, I)
        #plt.plot(times, anal_current)
        #plt.plot(times, anal_current_2)
        #plt.show()
        #plt.plot(V,I)
        #plt.show()
...
m, p, z=impedance_response(min_f=min_f, max_f=max_f, points_per_decade=points_per_decade, num_osc=param_list["num_peaks"], 
                    parameters=param_list,sim_class=sim_class, 
                    sf=1/param_list["sampling_freq"],amplitude=param_list["d_E"], noise=None)


#
#EIS().bode(np.column_stack((z.real, z.imag)), freqs, twinx=twinx, data_type="complex", compact_labels=True, ax=ax, label=amp)
#plt.show()
